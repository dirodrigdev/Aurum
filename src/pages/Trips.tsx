...
export const Trips = () => {
  ...
  const [form, setForm] = useState({
    ...
    // Flags (no afectan totales del viaje; solo futuras estadísticas)
   ...
   */
    ...
    setForm({
      ...
      excludeFromStats: Boolean((trip as any).exclude_from_trip_stats),
    });
  ...
  const handleSaveTrip = async () => {
    ...
    try {
      const tripData: any = {
        ...
        // ✅ solo para analítica futura (no cambia totales)
        exclude_from_trip_stats: Boolean((form as any).excludeFromStats),
        ...
        <Card className="mb-8 border-none shadow-2xl bg-white rounded-[2.5rem] overflow-hidden animate-in slide-in-from-top-6 duration-500">
          ...
          <div className="px-6 py-5">
            <div className="space-y-4">
              ...
              {/* Viaje sin estadísticas (solo afecta estadísticas futuras, no totales) */}
              <div className="rounded-2xl bg-slate-50/60 border border-slate-100 p-4">
                <div className="flex items-center justify-between gap-4">
                  <div className="min-w-0">
                    <p className="text-[10px] font-black uppercase tracking-widest text-slate-500">Viaje sin estadísticas</p>
                    {(form as any).excludeFromStats && (
                      <p className="mt-1 text-[11px] font-semibold text-slate-500">
                        No se usará en estadísticas futuras (hotel/vuelos). En Trips no cambia totales ni promedios.</p>
                    )}
                  </div>
                  ...
                  <button
                    type="button"
                    role="switch"
                    aria-checked={Boolean((form as any).excludeFromStats)}
                    onClick={() => setForm((prev) => ({ ...prev, excludeFromStats: !Boolean((prev as any).excludeFromStats) }))}
                    className={cn(
                      'relative w-11 h-6 rounded-full transition-all shrink-0',
                      Boolean((form as any).excludeFromStats) ? 'bg-slate-900' : 'bg-slate-200'
                    )}
                    title={Boolean((form as any).excludeFromStats) ? 'Excluido de estadísticas' : 'Incluido en estadísticas'}
                  >
                    <span
                      className={cn(
                        'absolute top-0.5 left-0.5 w-5 h-5 rounded-full bg-white shadow transition-transform',
                        Boolean((form as any).excludeFromStats) ? 'translate-x-5' : 'translate-x-0'
                      )}
                    />
                  </button>
                </div>
