// src/data/currencyMaster.ts
// Currency master (offline). Source: OpenExchangeRates currency list.
// Note: includes some non-ISO/commodity/crypto codes (e.g., XAU, XAG, BTC). That's ok for selection;
// your "strict ISO-only" rule can be tightened later if you want.
...
export const CURRENCY_MAP: Record<string, string> = CURRENCY_LIST.reduce((acc, c) => {
  acc[c.code] = c.name;
  return acc;
}, {} as Record<string, string>);

export const normalizeCurrencyCode = (input: string): string => {
  return (input || '').trim().toUpperCase();
};

export const isKnownCurrencyCode = (code: string): boolean => {
  const k = normalizeCurrencyCode(code);
  return !!(k && CURRENCY_MAP[k]);
};

export const getCurrencyLabel = (code: string): string => {
  const k = normalizeCurrencyCode(code);
  if (!k) return '';
  const name = CURRENCY_MAP[k];
