...
const SensitivityPageLazy = React.lazy(() =>
  import('./components/SensitivityPage').then((module) => ({ default: module.SensitivityPage })),
);
...
export function buildAggregateDataTrustVisual(verdict: AggregateDataTrustVerdict) {
  if (verdict.label === 'Online') {
    return { dotBackground: T.positive, dotBorder: T.positive };
  }
  if (verdict.label === 'Respaldo reciente') {
    return { dotBackground: T.warning, dotBorder: T.positive };
  }
  if (verdict.label === 'Respaldo antiguo') {
    return { dotBackground: '#f59e0b', dotBorder: '#f59e0b' };
  }
  if (verdict.label === 'No comparable') {
    return { dotBackground: T.negative, dotBorder: T.warning };
  }
  return { dotBackground: T.negative, dotBorder: T.negative };
}
...
export default function App() {
  ...
  const [authDiagnostics, setAuthDiagnostics] = useState<AurumIntegrationAuthBootstrapDiagnostics>({
    ...
    firebaseConfigAuthDomain: null,
    effectiveAuthDomain: null,
    expectedAuthHandlerUrl: null,
    currentHostname: null,
    isAuthDomainSameAsAppHost: false,
    usesAuthProxy: false,
    signInMethod: null,
  ...
  const m8InputFingerprint = useMemo<M8InputFingerprint>(() => buildM8InputFingerprint({
    ...
    authDiagnostics: {
      ...
      currentUserUid: authDiagnostics.currentUserUid,
      currentUserIsAnonymous: authDiagnostics.currentUserIsAnonymous,
      currentUserProviderIds: authDiagnostics.currentUserProviderIds,
      firebaseConfigAuthDomain: authDiagnostics.firebaseConfigAuthDomain,
      effectiveAuthDomain: authDiagnostics.effectiveAuthDomain,
      expectedAuthHandlerUrl: authDiagnostics.expectedAuthHandlerUrl,
      currentHostname: authDiagnostics.currentHostname,
      isAuthDomainSameAsAppHost: authDiagnostics.isAuthDomainSameAsAppHost,
      usesAuthProxy: authDiagnostics.usesAuthProxy,
      ...
      lastSignInAttemptMethod: authDiagnostics.lastSignInAttemptMethod,
      clickedGoogleAt: authDiagnostics.clickedGoogleAt,
      signOutAnonymousBeforeGoogle: authDiagnostics.signOutAnonymousBeforeGoogle,
      signOutAnonymousError: authDiagnostics.signOutAnonymousError,
      redirectPendingBeforeStart: authDiagnostics.redirectPendingBeforeStart,
      redirectStartedAt: authDiagnostics.redirectStartedAt,
      redirectReturnedAt: authDiagnostics.redirectReturnedAt,
      ...
      redirectResultProviderIds: authDiagnostics.redirectResultProviderIds,
      redirectResultErrorCode: authDiagnostics.redirectResultErrorCode,
      redirectResultErrorMessage: authDiagnostics.redirectResultErrorMessage,
      ...
      onAuthStateChangedEvents: authDiagnostics.onAuthStateChangedEvents,
  ...
  return (
    ...
    >
      ...
      >
        ...
        <span style={{ width: 10, height: 10, borderRadius: '50%', background: statusDotBackground || statusColor, boxShadow: `0 0 0 2px ${statusDotBorder}`, flex: '0 0 auto' }} />
      </button>
    </header>
  );
}
