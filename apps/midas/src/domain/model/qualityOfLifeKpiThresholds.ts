...
export type QualityOfLifeKpiThreshold = {
  ...
  label: 'Bueno' | 'Atención' | 'Crítico' | 'Informativo';
  explanation: string;
...
export function resolveQualityOfLifeKpiThreshold(
  ...
  if (kpiId === 'csr85_4') {
    ...
    if ((value as number) >= 0.65) return { status: 'yellow', label: 'Atención', explanation: 'La simulación logra calidad mínima aceptable, pero con margen todavía exigido.' };
    return { status: 'red', label: 'Crítico', explanation: 'Pocas trayectorias logran sostener una calidad de vida mínima aceptable.' };
  }
  ...
  if (kpiId === 'qualitySurvivalRate') {
    ...
    if ((value as number) >= 0.25) return { status: 'yellow', label: 'Atención', explanation: 'El filtro estricto de calidad todavía deja una zona exigida a revisar.' };
    return { status: 'red', label: 'Crítico', explanation: 'Muy pocas trayectorias pasan el filtro estricto de calidad; es una alerta de fragilidad.' };
  }
  ...
  if (kpiId === 'averageEffectiveSpendingRatio') {
    ...
    if ((value as number) >= 0.93) return { status: 'yellow', label: 'Atención', explanation: 'El gasto promedio se sostiene razonablemente, pero ya muestra recortes visibles.' };
    return { status: 'red', label: 'Crítico', explanation: 'El gasto promedio cae demasiado respecto del objetivo.' };
  }
  ...
  if (kpiId === 'severeCutYearsMean') {
    ...
    if ((years as number) <= 3) return { status: 'yellow', label: 'Atención', explanation: 'El tiempo promedio en recorte severo ya exige revisión, aunque todavía no es extremo.' };
    return { status: 'red', label: 'Crítico', explanation: 'La simulación pasa demasiado tiempo promedio en recorte severo.' };
  }
  ...
  if ((terminalRatio as number) < 0.5) {
    return { status: 'red', label: 'Crítico', explanation: 'El margen terminal es muy bajo y puede señalar poca holgura al cierre del horizonte.' };
  }
  if ((terminalRatio as number) > 2 && Number.isFinite(severeCutYears) && (severeCutYears as number) > 1) {
    return { status: 'yellow', label: 'Atención', explanation: 'Hay margen terminal alto junto con recortes relevantes; puede haber subuso del patrimonio.' };
  }
