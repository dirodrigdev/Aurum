...
function runObservedHistoricalPath(
  ...
  observedPath: number[][],
): Omit<WalkForwardResult, 'trainEnd' | 'testPeriod' | 'p10' | 'p50' | 'p90' | 'bucketReal' | 'probRuin' | 'errorSigned' | 'errorAbs' | 'bandWidthRel'> {
  const {
    capitalInitial: W0,
    weights,
    feeAnnual,
  ...
  return {
    realFinalWealth: sl.reduce((a, b) => a + b, 0),
    minWealth,
    maxDrawdown,
    monthsCut,
    monthsBelow12mSpend,
...
function evaluateWalkForward(
  label: 'motor1' | 'motor2',
  runner: (params: ModelParameters, trainData: number[][] | null, horizonMonths: number) => SimulationResults,
  ...
  const results = cuts.map((cut) => {
    ...
    const projected = runner(params, trainData, horizonMonths);
    ...
    const bucketReal = classifyBucket(observed.realFinalWealth, p10, p90);
    const errorSigned = observed.realFinalWealth !== 0 ? ((p50 / observed.realFinalWealth) - 1) : Number.POSITIVE_INFINITY;
    ...
    return {
      ...
      realFinalWealth: observed.realFinalWealth,
      p10,
      p50,
      p90,
      bucketReal,
...
function runMotor1Comparable(params: ModelParameters, trainData: number[][] | null): SimulationResults {
  return runSimulationCoreWithHistoricalData(params, trainData);
}

function runMotor2Comparable(params: ModelParameters): SimulationResults {
  // Motor 2 se valida tal como esta hoy: supuestos explicitos fijos,
  // sin reentrenamiento por ventana. No usa datos del test.
  return runSimulationParametric(params);
}
...
function runMotor1StabilitySummary(): { official: AuditSummary; seedRange: { prob: number; term: number }; blockRange: { prob: number; term: number }; weightingGap: { prob: number; term: number } } {
  ...
  const official = runMotor1Audit(base, { usePreprocess: true, useWeightedBootstrap: true });
...
function runMotor2StabilitySummary(): { official: AuditSummary; seedRange: { prob: number; term: number } } {
  const official = (() => {
    const result = runSimulationParametric(cloneParams(DEFAULT_PARAMETERS));
    return {
...
function buildScoreTable(motor1Summary: WalkForwardSummary, motor2Summary: WalkForwardSummary): ScoreRow[] {
  const m1Stability = runMotor1StabilitySummary();
  const m2Stability = runMotor2StabilitySummary();
  ...
  const buildAbsErrorScore = (summary: WalkForwardSummary) =>
    clampScore(100 - (summary.meanAbsoluteError * 100));
  ...
  const buildMotor2StabilityScore = () => {
    const official = m2Stability.official;
    const penalties = [
      (m2Stability.seedRange.prob / official.probRuin) * 100,
      (m2Stability.seedRange.term / official.terminalP50) * 100,
    ];
    const penalty = penalties.reduce((a, b) => a + b, 0) / penalties.length;
    return clampScore(100 - penalty);
  };
  ...
  const rows: ScoreRow[] = [
    {
      motor: 'Motor 1',
      coverageScore: buildCoverageScore(motor1Summary),
      biasScore: buildBiasScore(motor1Summary),
      absErrorScore: buildAbsErrorScore(motor1Summary),
      stabilityScore: buildMotor1StabilityScore(),
      totalScore: 0,
      comment: 'Buena cobertura historica, mediana pesimista, pero penalizado por sensibilidad estructural al bootstrap.',
    },
    {
      motor: 'Motor 2',
      coverageScore: buildCoverageScore(motor2Summary),
      biasScore: buildBiasScore(motor2Summary),
      absErrorScore: buildAbsErrorScore(motor2Summary),
      stabilityScore: buildMotor2StabilityScore(),
      totalScore: 0,
      comment: 'Mas estable por seed, pero si la cobertura walk-forward sale peor, pierde fuerza como motor base.',
    },
  ];

  rows.forEach((row) => {
    row.totalScore =
      (0.35 * row.coverageScore) +
      (0.25 * row.biasScore) +
      (0.20 * row.absErrorScore) +
      (0.20 * row.stabilityScore);
  });
...
const motor1 = evaluateWalkForward('motor1', (params, trainData) => runMotor1Comparable(params, trainData));
const motor2 = evaluateWalkForward('motor2', (params) => runMotor2Comparable(params));
const scoreTable = buildScoreTable(motor1.summary, motor2.summary);

console.log('\nTABLA A - WALK-FORWARD MOTOR 2');
console.table(motor2.cuts.map((cut) => ({
  ...
  error_signed: Number.isFinite(cut.errorSigned) ? fmtPct(cut.errorSigned) : 'inf',
  error_abs: Number.isFinite(cut.errorAbs) ? fmtPct(cut.errorAbs) : 'inf',
  band_width_rel: Number.isFinite(cut.bandWidthRel) ? fmtPct(cut.bandWidthRel) : 'inf',
})));

console.log('\nTABLA B - SUPERVIVENCIA INTERMEDIA MOTOR 2');
console.table(motor2.cuts.map((cut) => ({
  train_end: cut.trainEnd,
  test_period: cut.testPeriod,
...
console.log('\nTABLA C - RESUMEN AGREGADO MOTOR 2');
console.table([{
  within_p10_p90: fmtPct(motor2.summary.withinPct),
  below_p10: fmtPct(motor2.summary.belowPct),
  above_p90: fmtPct(motor2.summary.abovePct),
  mean_signed_error: fmtPct(motor2.summary.meanSignedError),
  mean_absolute_error: fmtPct(motor2.summary.meanAbsoluteError),
  mean_band_width_rel: fmtPct(motor2.summary.meanBandWidthRel),
}]);

console.log('\nTABLA D - SCORE COMPARATIVO MOTOR 1 VS MOTOR 2');
