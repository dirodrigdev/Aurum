...
function buildScoreTable(summaries: Record<string, WalkForwardSummary>): ScoreRow[] {
  ...
  return [
    ...
    },
    {
      ...
      totalScore: 0,
      comment: 'Motor central explicito: solo sirve si logra una zona util sin perder defendibilidad historica.',
    },
  ].map((row) => ({
    ...row,
...
const motor6Seeds = [11, 42, 84].map((seed) => {
  const params = cloneParams(DEFAULT_PARAMETERS);
  params.simulation.seed = seed;
  const result = runSimulationCentralV2Audit(params);
...
console.log('\nTABLA SEED MOTOR 6');
console.table(motor6Seeds.map((row) => ({
  seed: row.seed,
  probRuin: fmtPct(row.probRuin),
  terminalP50: fmtMM(row.terminalP50),
})));

console.log('\nTABLA WALK-FORWARD MOTOR 6');
console.table(motor6WalkForward.cuts.map((cut) => ({
...
console.log('\nTABLA RESUMEN AGREGADO MOTOR 6');
console.table([{
  within_p10_p90: fmtPct(motor6WalkForward.summary.withinPct),
  below_p10: fmtPct(motor6WalkForward.summary.belowPct),
  above_p90: fmtPct(motor6WalkForward.summary.abovePct),
  mean_signed_error: fmtPct(motor6WalkForward.summary.meanSignedError),
  mean_absolute_error: fmtPct(motor6WalkForward.summary.meanAbsoluteError),
  mean_band_width_rel: fmtPct(motor6WalkForward.summary.meanBandWidthRel),
}]);

console.log('\nSCORE COMPARATIVO CON MOTOR 6');
