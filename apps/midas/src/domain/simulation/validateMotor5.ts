...
function buildScoreTable(summaries: Record<string, WalkForwardSummary>): ScoreRow[] {
  ...
  const m5Stability = runSimpleStabilitySummary(runSimulationCentral);
  ...
  return [
    ...
    {
      motor: 'Motor 5',
      coverageScore: buildCoverageScore(summaries['Motor 5']),
      biasScore: buildBiasScore(summaries['Motor 5']),
      absErrorScore: buildAbsErrorScore(summaries['Motor 5']),
      stabilityScore: buildSimpleStabilityScore(m5Stability),
      totalScore: 0,
      comment: 'Candidato central solo si logra mejorar defendibilidad sin separarse de la evidencia historica.',
    },
  ].map((row) => ({
    ...row,
...
const baseComparison = [
  ...
  { motor: 'Motor 5', ...runSimulationCentralAudit(baseParams) },
];

const motor5Seeds = [11, 42, 84].map((seed) => {
  const params = cloneParams(DEFAULT_PARAMETERS);
  params.simulation.seed = seed;
  const result = runSimulationCentralAudit(params);
  return { seed, probRuin: result.probRuin, terminalP50: result.terminalP50 };
});

const motor5WalkForward = evaluateWalkForward((params) => runSimulationCentral(params));
...
const scoreTable = buildScoreTable({
  ...
  'Motor 5': motor5WalkForward.summary,
});
...
console.log('\nTABLA SEED MOTOR 5');
console.table(motor5Seeds.map((row) => ({
  seed: row.seed,
  probRuin: fmtPct(row.probRuin),
  terminalP50: fmtMM(row.terminalP50),
})));

console.log('\nTABLA WALK-FORWARD MOTOR 5');
console.table(motor5WalkForward.cuts.map((cut) => ({
...
console.log('\nTABLA RESUMEN AGREGADO MOTOR 5');
console.table([{
  within_p10_p90: fmtPct(motor5WalkForward.summary.withinPct),
  below_p10: fmtPct(motor5WalkForward.summary.belowPct),
  above_p90: fmtPct(motor5WalkForward.summary.abovePct),
  mean_signed_error: fmtPct(motor5WalkForward.summary.meanSignedError),
  mean_absolute_error: fmtPct(motor5WalkForward.summary.meanAbsoluteError),
  mean_band_width_rel: fmtPct(motor5WalkForward.summary.meanBandWidthRel),
}]);

console.log('\nSCORE COMPARATIVO CON MOTOR 5');
