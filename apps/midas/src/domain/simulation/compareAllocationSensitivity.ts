...
function buildAllocationScenarios(): AllocationScenario[] {
  const current = DEFAULT_PARAMETERS.weights;
  const equityTotal = current.rvGlobal + current.rvChile;
  const fixedIncomeTotal = current.rfGlobal + current.rfChile;
  const equityGlobalShare = current.rvGlobal / equityTotal;
  const equityChileShare = current.rvChile / equityTotal;
  const fiGlobalShare = current.rfGlobal / fixedIncomeTotal;
  const fiChileShare = current.rfChile / fixedIncomeTotal;

  return [
    {
      label: 'Conservador',
      weights: {
        rvGlobal: 0.20 * equityGlobalShare,
        rvChile: 0.20 * equityChileShare,
        rfGlobal: 0.80 * fiGlobalShare,
        rfChile: 0.80 * fiChileShare,
      },
    },
    {
      label: 'Actual',
    ...
    },
    {
      label: 'Agresivo',
      weights: {
        rvGlobal: 0.80 * equityGlobalShare,
        rvChile: 0.80 * equityChileShare,
        rfGlobal: 0.20 * fiGlobalShare,
        rfChile: 0.20 * fiChileShare,
      },
    },
  ];
}

const allocationScenarios = buildAllocationScenarios();
const motors = [
  'Motor 1',
  'Motor 2',
  'Motor 3',
...
const results = motors.flatMap((motor) => allocationScenarios.map((scenario) => {
  ...
  return {
    motor,
    allocation: scenario.label,
    audit,
  };
}));

console.log('\nTABLA PRINCIPAL - SENSIBILIDAD DE ASSET ALLOCATION');
