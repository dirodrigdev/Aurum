...
assert(source.includes('Ajustes futuros pendientes:'));
...
assert(source.includes('Agrega entradas o salidas T0/futuras como borrador local visible. No entran al motor M8 canónico hasta llevarlas al Modelo Base.'));
...
assert(source.includes('Los ajustes futuros visibles no cambian los recursos habilitados hoy ni forman parte de la corrida canónica vigente hasta llevarlos al Modelo Base.'));
...
assert(source.includes('Borrador local'));
assert(source.includes('Cambios pendientes · Guardar y salir conserva este borrador local, pero no recalcula ni actualiza el fingerprint canónico.'));
assert(source.includes('Guardar borrador local'));
assert(source.includes('No cambia fingerprint canónico'));
assert(source.includes('No afecta resultado vigente'));
