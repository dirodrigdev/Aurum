...
assert(appSource.includes('() => computeEffectiveEngineInputHashForParams(simParams)'));
...
assert(source.includes('order: isMobileViewport ? 1 : 2'));
assert(source.includes('order: isMobileViewport ? 4 : 1'));
assert(source.includes('order: isMobileViewport ? 2 : 0'));
...
assert(source.includes('Ligado al fingerprint vigente'));
