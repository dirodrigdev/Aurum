...
export const HistoricalFxCorrectionConsole: React.FC<{
  ...
  const apply = async () => {
    ...
    try {
      ...
      if (result.status !== 'applied_verified' || !result.verification?.after?.projectId || !ratesVerified || !netsVerified || reread.fingerprint !== result.fingerprint) throw Object.assign(new Error(`La relectura UI no verificó la operación ${result.operationId}.`), { code: 'post_write_verification_failed' });
      setRead(reread);
