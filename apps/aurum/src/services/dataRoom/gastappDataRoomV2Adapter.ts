...
const normalizePeriodSummary = (id: string, raw: Record<string, unknown>): GastappDataRoomV2PeriodSummary => ({
  ...
  normalizedRowsAmountEur: readNumber(raw.normalizedRowsAmountEur ?? raw.normalized_rows_amount_eur),
  reconciliationDeltaEur: readNumber(raw.reconciliationDeltaEur ?? raw.reconciliation_delta_eur),
  dayToDayEur: readNumber(raw.day_to_day_eur ?? raw.dayToDayEur),
  projectsEur: readNumber(raw.projects_eur ?? raw.projectsEur),
  tripsEur: readNumber(raw.trips_eur ?? raw.tripsEur),
  classifiedAmountEur: readNumber(raw.classifiedAmountEur ?? raw.classified_amount_eur),
  unclassifiedAmountEur: readNumber(raw.unclassifiedAmountEur ?? raw.unclassified_amount_eur),
...
const normalizeRow = (id: string, raw: Record<string, unknown>): GastappDataRoomV2Row => ({
  ...
  transactionOwner: readString(raw.transaction_owner) || readString(raw.transactionOwner),
  transactionOwnerSource: readString(raw.transaction_owner_source) || readString(raw.transactionOwnerSource),
