...
const buildGastappDataRoomV2PeriodSummaryRows = (rows: GastappDataRoomV2PeriodSummary[]) =>
  rows.map((row) => ({
    ...
    normalizedRowsAmountEur: row.normalizedRowsAmountEur,
    reconciliationDeltaEur: row.reconciliationDeltaEur,
    dayToDayEur: row.dayToDayEur,
    projectsEur: row.projectsEur,
    tripsEur: row.tripsEur,
    classifiedAmountEur: row.classifiedAmountEur,
    unclassifiedAmountEur: row.unclassifiedAmountEur,
...
const buildGastappDataRoomV2RowCsvRows = (rows: GastappDataRoomV2Row[]) =>
  rows.map((row) => ({
    ...
    transactionOwnerSource: row.transactionOwnerSource,
...
export const buildFinancialDataRoom = (input: {
  ...
  if (input.gastappDataRoomV2) {
    ...
    files.push(toCsvFile('gastapp_data_room_v2_period_summaries.csv', [
      ...
      'readinessStatus',
      'officialAmountEur',
      'normalizedRowsAmountEur',
      'reconciliationDeltaEur',
      'dayToDayEur',
      'projectsEur',
      'tripsEur',
      'classifiedAmountEur',
      'unclassifiedAmountEur',
      'canonicalRowCount',
      'rowCount',
      'periodStart',
      'periodEnd',
