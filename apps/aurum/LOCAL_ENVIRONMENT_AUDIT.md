...
## Entorno E2E autenticado local

El smoke autenticado ya no usa producción, Google, Vercel ni MIDAS. `VITE_E2E_USE_FIREBASE_EMULATOR=true` activa exclusivamente el proyecto ficticio `aurum-e2e-local`; la inicialización rechaza cualquier projectId distinto. En ese modo, Auth y Firestore se conectan a `127.0.0.1`, se autentica un usuario local determinista y las integraciones de MIDAS, Fintoc, GastApp, FX externo y correcciones históricas se deshabilitan o sustituyen por fixtures locales.
